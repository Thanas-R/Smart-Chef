...
export const RecipeModal = ({
  ...
  const handleGenerateDetails = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-recipe-details', {
        body: {
  ...
  const handleGenerateInstructions = async () => {
    ...
    try {
      const instructions = await api.generateInstructions(displayRecipe.id, displayRecipe.title, displayRecipe.ingredients);
      setLocalRecipe({
        ...displayRecipe,
        instructions
      });
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] bg-card border-2 border-border rounded-3xl">
        ...
        <ScrollArea className="h-[75vh] pr-4">
          <div className="space-y-8">
            {isGeneratingDetails && <div className="flex items-center justify-center py-8 bg-primary/5 rounded-2xl">
            ...
            <div className="flex flex-wrap gap-3 items-center pb-6 border-b border-border">
              ...
              {displayRecipe.cuisine && <div className="bg-secondary/50 px-4 py-2 rounded-full">
                  <span className="text-sm font-semibold text-foreground">{displayRecipe.cuisine}</span>
                </div>}
              {displayRecipe.servings && <div className="bg-secondary/50 px-4 py-2 rounded-full">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                return <div key={`${ingredient}-${idx}`} className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-colors ${hasIngredient ? "bg-success/5 border-success/30" : "bg-muted/30 border-border"}`}>
                      {hasIngredient ? <Check className="w-5 h-5 text-success flex-shrink-0" /> : <X className="w-5 h-5 text-muted-foreground flex-shrink-0" />}
                      <span className={`text-sm font-semibold ${hasIngredient ? "text-foreground" : "text-muted-foreground"}`}>
                        {ingredient}
                      </span>
                    </div>;
              })}
            ...
            {/* Equipment */}
            {displayRecipe.equipment && displayRecipe.equipment.length > 0 && <div>
                <div className="flex items-center gap-2 mb-4">
                  <Utensils className="w-5 h-5 text-primary" />
                  <h3 className="text-2xl font-serif font-bold text-foreground">Equipment Needed</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {displayRecipe.equipment.map((item, idx) => <div key={idx} className="bg-secondary/50 px-4 py-2 rounded-full border border-border">
            ...
            <div>
              <div className="flex items-center justify-between mb-5">
                ...
                {!hasInstructions && !isGeneratingDetails && <Button onClick={handleGenerateInstructions} disabled={isGenerating} className="gap-2">
                    <Sparkles className="w-4 h-4" />
                    {isGenerating ? "Generating..." : "Generate Instructions"}
                  </Button>}
              </div>
              {hasInstructions ? <div className="space-y-5">
                  {displayRecipe.instructions.map((instruction, idx) => <div key={idx} className="flex gap-4 group">
                ...
                </div> : !isGeneratingDetails && <div className="text-center py-8 bg-muted/30 rounded-2xl border-2 border-dashed border-border">
            ...
            {/* Chef Tips */}
            {displayRecipe.chef_tips && displayRecipe.chef_tips.length > 0 && <div>
                <div className="flex items-center gap-2 mb-4">
                  <Lightbulb className="w-5 h-5 text-primary" />
                  <h3 className="text-2xl font-serif font-bold text-foreground">Chef's Tips</h3>
                </div>
                <div className="space-y-3">
                  {displayRecipe.chef_tips.map((tip, idx) => <div key={idx} className="flex gap-3 p-4 bg-primary/5 rounded-xl border border-primary/20">
